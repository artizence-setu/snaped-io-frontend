  ...
  const fetchVoices = async () => {
    try {
      const response = await fetch("/api/voices", {
        ...
        body: JSON.stringify({country : selectedLanguage , region : selectedRegion}),
      })
      const data = await response.json()
      ...
      if (!isTextProcessed) {
        ...
        if (text && selectedVoice) {
          ...
          if (ttsData.audioUrl) {
            ...
            setSuccessMessage("Sound generated successfully. You can now proceed to generate the video.")
          }
        } else {
    ...
    <Card className="py-4">
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label htmlFor="text" className="block text-sm font-medium text-gray-200 mb-1">
              Script
            </label>
            <textarea
              id="text"
          ...
          </div>
          {
            ...
            <div>
              ...
              <Select value={selectedRegion} onValueChange={handleRegionChange}>
                ...
                <SelectContent>
                  <SelectItem value="auto-detect">Auto-detect</SelectItem>
                  {languages.filter(v=>v.value===selectedLanguage).map((regions) => {
          ...
          {voices.length > 0 && (
            <>
              <div>
                <label htmlFor="voiceCategory" className="block text-sm font-medium text-gray-200 mb-1">
                  Select a voice category
                </label>
                <Select value={selectedVoiceCategory} onValueChange={handleVoiceCategoryChange}>
                  <SelectTrigger>
              ...
              <div>
                <label htmlFor="voice" className="block text-sm font-medium text-gray-200 mb-1">
                  Select a voice
                </label>
                <Select value={selectedVoice} onValueChange={handleVoiceChange}>
                  ...
                  <SelectContent>
                    {voices
                      .find((category) => category.value === selectedVoiceCategory)
                      ?.children.map((voice) => (
