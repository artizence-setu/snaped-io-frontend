...
export default function VideoGeneration({ text, language, audioUrl, selectedVoiceAnchor , setVideoUrl , selectedVoice , setSelectedVoice, setAudioUrl , setSelectedAnchor , isGenerating , setIsGenerating , setGenerationProgress}: VideoGenerationProps) {
  ...
  const handleGenerateVideo = async () => {
    if (!text || !language) {
      ...
      setError("Please complete all previous steps before generating the video.")
      return
    }
  ...
  return (
    <div className="space-y-4">
      {error && (
        <div className="mx-5 bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative" role="alert">
      ...
      <div className="flex items-center justify-end mr-2">
            <Button onClick={handleGenerateVideo}
                disabled={isGenerating} className="px-4 py-2 mt-4">
              <RiGeminiLine className={`mr-1 size-4 ${isGenerating?"animate-spin" : "animate-none"}`} />
